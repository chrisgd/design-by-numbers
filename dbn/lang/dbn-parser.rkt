...
(define dbn-parser
  (parser
   ...
   ; we expect 2 shift reduce conflicts due to the fact that a function
   ; call can be followed by another function call, and in this case
   ; we just don't know if we should shift or reduce on an ID--we fix this
   ; in a later pass
   ;(expected-SR-conflicts 2)
